...
/*!
    ...
    \pre #ts_initialize or #ts_initialize_with_path, #ts_mocap_start_streaming
    \param[in] skeleton handle to mocap data
    \param[in] index bone index to receive data from
    \param[out] bone mocap bone data
    \return #TsStatusCode
    \sa TsBoneIndex
*/
...
/*!
    ...
    \pre #ts_initialize or #ts_initialize_with_path, #ts_mocap_start_streaming
    \param[in] skeleton handle to raw mocap data
    \param[in] index bone index to receive data from
    \param[out] bone raw mocap bone data
    \return #TsStatusCode
    \sa TsBoneIndex
*/
...
/*!
    \brief Retrieves skeleton biomechanical angle data by its index.

    \pre #ts_initialize or #ts_initialize_with_path, #ts_mocap_start_streaming
    \param[in] skeleton handle to access biomechanical angle
    \param[in] index biomechanical index to receive angle from
    \param[out] angle biomechanical angle value in degrees
    \return #TsStatusCode
    \sa TsBiomechanicalIndex
*/
TS_API TsStatusCode TS_CALL ts_mocap_skeleton_get_biomechanical_angle(const TsMocapSkeleton skeleton, TsBiomechanicalIndex index, float* angle);

/*!
    \brief Retrieves skeleton foot contacts.

    \pre #ts_initialize or #ts_initialize_with_path, #ts_mocap_start_streaming
    \param[in] skeleton handle to access foot contact
    \param[out] left foot contact data. 1 - detected, 0 - not detected, -1 - undefined (not presented)
    \param[out] right foot contact data. 1 - detected, 0 - not detected, -1 - undefined (not presented)
    \return #TsStatusCode
    \sa TsMocapSkeletonContact
*/
TS_API TsStatusCode TS_CALL ts_mocap_skeleton_get_foot_contacts(const TsMocapSkeleton skeleton, int* left_foot, int* right_foot);
